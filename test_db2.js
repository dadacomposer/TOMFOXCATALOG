...
async function check() {
  const { data, error } = await supabase.from('tracks').select('id, file_name, track_type, waveform_data, deleted_at').is('waveform_data', null);
  if (error) console.error(error);
  console.log("Tracks without waveform:");
