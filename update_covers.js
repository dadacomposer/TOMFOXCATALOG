...
async function getUnsplashId(query) {
  ...
  return '1485846234645-a62644f84728';
}
...
const map = [
  ...
  { title: "Ethereal dreamy", id: "1478760327741-6cb31eda2283" }, // Balloons
  ...
  { title: "Drop the Beat", id: "1517230878791-4d28214057c2" }, // DJ console
  ...
  { title: "Film", id: "1485846234645-a62644f84728" }, // New Music cover
...
async function run() {
  ...
  // First handle Vox
  const voxSvg = `data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect width="100" height="100" fill="%23aec6cf" /><image href="https://upload.wikimedia.org/wikipedia/commons/a/a2/Vox_logo.svg" x="20" y="35" width="60" height="30" /></svg>`;
  sql += `UPDATE playlists SET cover_url = '${voxSvg}' WHERE title ILIKE '%Vox Borders Sessions%';\n`;
  ...
  for (const item of map) {
    ...
    if (item.id) {
      ...
      const id = await getUnsplashId(item.query);
  ...
  fs.writeFileSync('update_covers_script.sql', sql);
  console.log("SQL file generated: update_covers_script.sql");
}
